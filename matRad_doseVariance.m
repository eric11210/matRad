...
dGrad = zeros(dij.numOfVoxels,size(apertureInfo.apertureVector,1));

% allocate mean of squared dose
d2 = zeros(dij.numOfVoxels,1);

% allocate gradient of mean of squared dose
d2Grad = zeros(dij.numOfVoxels,size(apertureInfo.apertureVector,1));
...
for i1 = 1:numel(apertureInfo.beam)
    ...
    for i2 = 1:numel(apertureInfo.beam)
        ...
        for phase_I1 = 1:apertureInfo.numPhases
            ...
            for phase_F1 = 1:apertureInfo.numPhases
                ...
                for phase_I2 = 1:apertureInfo.numPhases
                    ...
                    for phase_F2 = 1:apertureInfo.numPhases
                        ...
                        d2_II = dij.scaleFactor.^2 .* pNorm .* (dij.physicalDose{phase_I1}(:,currBixelIx1) * w_I1) .* (dij.physicalDose{phase_I2} * w_I2);
                        d2_IF = dij.scaleFactor.^2 .* pNorm .* (dij.physicalDose{phase_I1}(:,currBixelIx1) * w_I1) .* (dij.physicalDose{phase_F2} * w_F2);
                        d2_FI = dij.scaleFactor.^2 .* pNorm .* (dij.physicalDose{phase_F1}(:,currBixelIx1) * w_F1) .* (dij.physicalDose{phase_I2} * w_I2);
                        d2_FF = dij.scaleFactor.^2 .* pNorm .* (dij.physicalDose{phase_F1}(:,currBixelIx1) * w_F1) .* (dij.physicalDose{phase_F2} * w_F2);
                        ...
                        pNormGrad(tIx_Vec) = sum(apertureInfo.propVMAT.jacobT(:,(iFirst+1):(iLast-1)),2).*Pij_transT12_dot(phase_F1,phase_I2)./Pi_T2(phase_I2) - ...
                            sum(apertureInfo.propVMAT.jacobT(:,1:(iLast-1)),2).*Pi_T2_dot.*Pij_transT12(phase_F1,phase_I2)./(Pi_T2(phase_I2).^2);
                        
                        % calculate gradients of mean squared doses
                        d2Grad_II = dij.scaleFactor.^2 .* ( ((dij.physicalDose{phase_I1}(:,currBixelIx1) * w_I1) .* (dij.physicalDose{phase_I2} * w_I2)) * pNormGrad + ...
                            pNorm .* (dij.physicalDose{phase_I1}(:,currBixelIx1) * j_I1') .* repmat((dij.physicalDose{phase_I2} * w_I2),1,size(apertureInfo.apertureVector,1)) + ...
                            pNorm .* repmat((dij.physicalDose{phase_I1}(:,currBixelIx1) * w_I1),1,size(apertureInfo.apertureVector,1)) .* (dij.physicalDose{phase_I2} * j_I2') );
                        
                        d2Grad_IF = dij.scaleFactor.^2 .* ( ((dij.physicalDose{phase_I1}(:,currBixelIx1) * w_I1) .* (dij.physicalDose{phase_F2} * w_F2)) * pNormGrad + ...
                            pNorm .* (dij.physicalDose{phase_I1}(:,currBixelIx1) * j_I1') .* repmat((dij.physicalDose{phase_F2} * w_F2),1,size(apertureInfo.apertureVector,1)) + ...
                            pNorm .* repmat((dij.physicalDose{phase_I1}(:,currBixelIx1) * w_I1),1,size(apertureInfo.apertureVector,1)) .* (dij.physicalDose{phase_F2} * j_F2') );
                        
                        d2Grad_FI = dij.scaleFactor.^2 .* ( ((dij.physicalDose{phase_F1}(:,currBixelIx1) * w_F1) .* (dij.physicalDose{phase_I2} * w_I2)) * pNormGrad + ...
                            pNorm .* (dij.physicalDose{phase_F1}(:,currBixelIx1) * j_F1') .* repmat((dij.physicalDose{phase_I2} * w_I2),1,size(apertureInfo.apertureVector,1)) + ...
                            pNorm .* repmat((dij.physicalDose{phase_F1}(:,currBixelIx1) * w_F1),1,size(apertureInfo.apertureVector,1)) .* (dij.physicalDose{phase_I2} * j_I2') );
                        
                        d2Grad_FF = dij.scaleFactor.^2 .* ( ((dij.physicalDose{phase_F1}(:,currBixelIx1) * w_F1) .* (dij.physicalDose{phase_F2} * w_F2)) * pNormGrad + ...
                            pNorm .* (dij.physicalDose{phase_F1}(:,currBixelIx1) * j_F1') .* repmat((dij.physicalDose{phase_F2} * w_F2),1,size(apertureInfo.apertureVector,1)) + ...
                            pNorm .* repmat((dij.physicalDose{phase_F1}(:,currBixelIx1) * w_F1),1,size(apertureInfo.apertureVector,1)) .* (dij.physicalDose{phase_F2} * j_F2') );
            ...
            % only need to do this once per beam angle
            if i2 == 1
                
                % d = d + dij.scaleFactor .*
                % dij.physicalDose{phase_I1}(:,currBixelIx1) * apertureInfo.bixelWeights{phase_I1}(currBixelIx1);
                dGrad = dGrad + dij.scaleFactor .* dij.physicalDose{phase_I1}(:,currBixelIx1) * apertureInfo.bixelJApVec{phase_I1}(:,currBixelIx1)';
