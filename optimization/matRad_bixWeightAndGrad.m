...
phase_I         = variables.phase_I;
phase_F         = variables.phase_F;
...
w{phase_I}(currBixelIx) = w{phase_I}(currBixelIx) + shapeMap(shapeMapIx).*weight_I.*weightFactor_I.*probability;
shapeMapW{phase_I} = shapeMapW{phase_I}+shapeMap.*weight_I.*weightFactor_I.*probability;
...
if calcOptions.saveJacobian
    ...
    if calcOptions.DAOBeam
        ...
        % wrt last weight
        bixelJApVec_vec{phase_I}(bixelJApVec_offset_temp+(1:numSaveBixel)) = fracFromLastOpt*(time./time_last)*shapeMap(shapeMapIx).*weightFactor_I.*probability./jacobiScale_last_I;
        ...
        % wrt next weight
        bixelJApVec_vec{phase_I}(bixelJApVec_offset_temp+(1:numSaveBixel)) = (1-fracFromLastOpt)*(time./time_next)*shapeMap(shapeMapIx).*weightFactor_I.*probability./jacobiScale_next_I;
...
%%%%% now do phase_F %%%%%
%
% leaves sweep from _M to _F, with weight weight_F.*weightFactor_F
%
...
%calculate fraction of fluence uncovered by left leaf
%initial computation
uncoveredByLeftLeaf = bsxfun(@minus,centres,leftLeafPosM)./repmat(leftLeafPosF-leftLeafPosM,1,numBix);
%correct for overshoot in initial and final leaf positions
uncoveredByLeftLeaf(xPosLinearIndLeftLeafM) = uncoveredByLeftLeaf(xPosLinearIndLeftLeafM) + (leftLeafPosM-edges_l(xPosIndLeftLeafM)').^2./((leftLeafPosF-leftLeafPosM).*(widths(xPosIndLeftLeafM)').*2);
uncoveredByLeftLeaf(xPosLinearIndLeftLeafF) = uncoveredByLeftLeaf(xPosLinearIndLeftLeafF) - (edges_r(xPosIndLeftLeafF)'-leftLeafPosF).^2./((leftLeafPosF-leftLeafPosM).*(widths(xPosIndLeftLeafF)').*2);
%round <0 to 0, >1 to 1
uncoveredByLeftLeaf(uncoveredByLeftLeaf < 0) = 0;
uncoveredByLeftLeaf(uncoveredByLeftLeaf > 1) = 1;
...
%calculate fraction of fluence covered by right leaf
%initial computation
coveredByRightLeaf = bsxfun(@minus,centres,rightLeafPosM)./repmat(rightLeafPosF-rightLeafPosM,1,numBix);
%correct for overshoot in initial and final leaf positions
coveredByRightLeaf(xPosLinearIndRightLeafM) = coveredByRightLeaf(xPosLinearIndRightLeafM) + (rightLeafPosM-edges_l(xPosIndRightLeafM)').^2./((rightLeafPosF-rightLeafPosM).*(widths(xPosIndRightLeafM)').*2);
coveredByRightLeaf(xPosLinearIndRightLeafF) = coveredByRightLeaf(xPosLinearIndRightLeafF) - (edges_r(xPosIndRightLeafF)'-rightLeafPosF).^2./((rightLeafPosF-rightLeafPosM).*(widths(xPosIndRightLeafF)').*2);
%round <0 to 0, >1 to 1
coveredByRightLeaf(coveredByRightLeaf < 0) = 0;
coveredByRightLeaf(coveredByRightLeaf > 1) = 1;
...
dUl_dLM = bsxfun(@minus,centres,leftLeafPosF)./(repmat(leftLeafPosF-leftLeafPosM,1,numBix)).^2;
dUl_dLF = bsxfun(@minus,leftLeafPosM,centres)./(repmat(leftLeafPosF-leftLeafPosM,1,numBix)).^2;

dCr_dRM = bsxfun(@minus,centres,rightLeafPosF)./(repmat(rightLeafPosF-rightLeafPosM,1,numBix)).^2;
dCr_dRF = bsxfun(@minus,rightLeafPosM,centres)./(repmat(rightLeafPosF-rightLeafPosM,1,numBix)).^2;

dUl_dLM(xPosLinearIndLeftLeafM) = dUl_dLM(xPosLinearIndLeftLeafM) + ((leftLeafPosM-edges_l(xPosIndLeftLeafM)').*(2*leftLeafPosF-leftLeafPosM-edges_l(xPosIndLeftLeafM)'))./((leftLeafPosF-leftLeafPosM).^2.*(widths(xPosIndLeftLeafM)').*2);
dUl_dLF(xPosLinearIndLeftLeafM) = dUl_dLF(xPosLinearIndLeftLeafM) - ((leftLeafPosM-edges_l(xPosIndLeftLeafM)').^2)./((leftLeafPosF-leftLeafPosM).^2.*(widths(xPosIndLeftLeafM)').*2);
dUl_dLM(xPosLinearIndLeftLeafF) = dUl_dLM(xPosLinearIndLeftLeafF) - ((edges_r(xPosIndLeftLeafF)'-leftLeafPosF).^2)./((leftLeafPosF-leftLeafPosM).^2.*(widths(xPosIndLeftLeafF)').*2);
dUl_dLF(xPosLinearIndLeftLeafF) = dUl_dLF(xPosLinearIndLeftLeafF) + ((edges_r(xPosIndLeftLeafF)'-leftLeafPosF).*(leftLeafPosF+edges_r(xPosIndLeftLeafF)'-2*leftLeafPosM))./((leftLeafPosF-leftLeafPosM).^2.*(widths(xPosIndLeftLeafF)').*2);

dCr_dRM(xPosLinearIndRightLeafM) = dCr_dRM(xPosLinearIndRightLeafM) + ((rightLeafPosM-edges_l(xPosIndRightLeafM)').*(2*rightLeafPosF-rightLeafPosM-edges_l(xPosIndRightLeafM)'))./((rightLeafPosF-rightLeafPosM).^2.*(widths(xPosIndRightLeafM)').*2);
dCr_dRF(xPosLinearIndRightLeafM) = dCr_dRF(xPosLinearIndRightLeafM) - ((rightLeafPosM-edges_l(xPosIndRightLeafM)').^2)./((rightLeafPosF-rightLeafPosM).^2.*(widths(xPosIndRightLeafM)').*2);
dCr_dRM(xPosLinearIndRightLeafF) = dCr_dRM(xPosLinearIndRightLeafF) - ((edges_r(xPosIndRightLeafF)'-rightLeafPosF).^2)./((rightLeafPosF-rightLeafPosM).^2.*(widths(xPosIndRightLeafF)').*2);
dCr_dRF(xPosLinearIndRightLeafF) = dCr_dRF(xPosLinearIndRightLeafF) + ((edges_r(xPosIndRightLeafF)'-rightLeafPosF).*(rightLeafPosF+edges_r(xPosIndRightLeafF)'-2*rightLeafPosM))./((rightLeafPosF-rightLeafPosM).^2.*(widths(xPosIndRightLeafF)').*2);

for k = 1:n
    dUl_dLM(k,1:(xPosIndLeftLeafM(k)-1)) = 0;
    dUl_dLF(k,1:(xPosIndLeftLeafM(k)-1)) = 0;
    dUl_dLM(k,(xPosIndLeftLeafF(k)+1):numBix) = 0;
    dUl_dLF(k,(xPosIndLeftLeafF(k)+1):numBix) = 0;
    
    if xPosIndLeftLeafM(k) >= xPosIndLeftLeafF(k)
        % in discrete aperture, the xPosIndLeftLeafM is greater than
        % xPosIndLeftLeafF when leaf positions are at a bixel boundary
        
        %19 July 2017 in journal
        dUl_dLM(k,xPosIndLeftLeafM(k)) = -1/(2*widths(xPosIndLeftLeafM(k))');
        dUl_dLF(k,xPosIndLeftLeafF(k)) = -1/(2*widths(xPosIndLeftLeafF(k))');
        if leftLeafPosF(k)-leftLeafPosM(k) <= eps(max(lim_r))
            uncoveredByLeftLeaf(k,xPosIndLeftLeafM(k)) = (edges_r(xPosIndLeftLeafM(k))-leftLeafPosM(k))./widths(xPosIndLeftLeafM(k));
            uncoveredByLeftLeaf(k,xPosIndLeftLeafF(k)) = (edges_r(xPosIndLeftLeafF(k))-leftLeafPosF(k))./widths(xPosIndLeftLeafF(k));
        end
    end
    
    dCr_dRM(k,1:(xPosIndRightLeafM(k)-1)) = 0;
    dCr_dRF(k,1:(xPosIndRightLeafM(k)-1)) = 0;
    dCr_dRM(k,(xPosIndRightLeafF(k)+1):numBix) = 0;
    dCr_dRF(k,(xPosIndRightLeafF(k)+1):numBix) = 0;
    
    if xPosIndRightLeafM(k) >= xPosIndRightLeafF(k)
        dCr_dRM(k,xPosIndRightLeafM(k)) = -1/(2*widths(xPosIndRightLeafM(k))');
        dCr_dRF(k,xPosIndRightLeafF(k)) = -1/(2*widths(xPosIndRightLeafF(k))');
        if rightLeafPosF(k)-rightLeafPosM(k) <= eps(max(lim_r))
            coveredByRightLeaf(k,xPosIndRightLeafM(k)) = (edges_r(xPosIndRightLeafM(k))-rightLeafPosM(k))./widths(xPosIndRightLeafM(k));
...
currBixelIx = bixelIndMap(shapeMapIx);
w{phase_F}(currBixelIx) = w{phase_F}(currBixelIx) + shapeMap(shapeMapIx).*weight_F.*weightFactor_F.*probability;
shapeMapW{phase_F} = shapeMapW{phase_F}+shapeMap.*weight_F.*weightFactor_F.*probability;

%% save the gradients

if calcOptions.saveJacobian
    
    bixelJApVec_offset_temp = bixelJApVec_offset{phase_F};
    numSaveBixel = nnz(shapeMapIx);
    ...
    if calcOptions.DAOBeam
        ...
        % wrt last weight
        bixelJApVec_vec{phase_F}(bixelJApVec_offset_temp+(1:numSaveBixel)) = fracFromLastOpt*(time./time_last)*shapeMap(shapeMapIx).*weightFactor_F.*probability./jacobiScale_last_F;
        %bixelJApVec_vec(bixelJApVec_offset+(1:numSaveBixel)) = (updatedInfo.beam(i).doseAngleBordersDiff*fracFromLastOpt*updatedInfo.beam(apertureInfo.beam(i).lastOptIndex).gantryRot ...
        %/(updatedInfo.beam(apertureInfo.beam(i).lastOptIndex).doseAngleBordersDiff*updatedInfo.beam(i).gantryRot))*updatedInfo.beam(i).shape(j).shapeMap(shapeMapIx) ...
        %./ apertureInfo.beam(apertureInfo.beam(i).lastOptIndex).shape(1).jacobiScale;
        bixelJApVec_i{phase_F}(bixelJApVec_offset_temp+(1:numSaveBixel)) = DAOindex_last+(phase_F-1)*totalNumOfShapes;
        bixelJApVec_j{phase_F}(bixelJApVec_offset_temp+(1:numSaveBixel)) = bixelIndMap(shapeMapIx);
        bixelJApVec_offset_temp = bixelJApVec_offset_temp+numSaveBixel;
        
        % wrt next weight
        bixelJApVec_vec{phase_F}(bixelJApVec_offset_temp+(1:numSaveBixel)) = (1-fracFromLastOpt)*(time./time_next)*shapeMap(shapeMapIx).*weightFactor_F.*probability./jacobiScale_next_F;
        %bixelJApVec_vec(bixelJApVec_offset+(1:numSaveBixel)) = (updatedInfo.beam(i).doseAngleBordersDiff*(1-fracFromLastOpt)*updatedInfo.beam(apertureInfo.beam(i).nextOptIndex).gantryRot ...
        %/(updatedInfo.beam(apertureInfo.beam(i).nextOptIndex).doseAngleBordersDiff*updatedInfo.beam(i).gantryRot))*updatedInfo.beam(i).shape(j).shapeMap(shapeMapIx) ...
        %./ apertureInfo.beam(apertureInfo.beam(i).nextOptIndex).shape(1).jacobiScale;
        bixelJApVec_i{phase_F}(bixelJApVec_offset_temp+(1:numSaveBixel)) = DAOindex_next+(phase_F-1)*totalNumOfShapes;
        bixelJApVec_j{phase_F}(bixelJApVec_offset_temp+(1:numSaveBixel)) = bixelIndMap(shapeMapIx);
        bixelJApVec_offset_temp = bixelJApVec_offset_temp+numSaveBixel;
