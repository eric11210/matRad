    ...
    if ~apertureInfo.runVMAT || (apertureInfo.runVMAT && apertureInfo.propVMAT.beam(i).DAOBeam)
        ...
        for j = 1:apertureInfo.beam(i).numOfShapes
            ...
            if ~apertureInfo.propVMAT.continuousAperture
                ...
                apertureInfo.beam(i).shape(j).jacobiScale = (dijScaleFactor./apertureInfo.beam(i).shape(j).weight).*sqrt(sum(apertureInfo.beam(i).shape(j).shapeMap(:).^2))./apertureInfo.beam(i).shape(j).sqrtSumGradSq;
            end
