...
if recalc.interpNew
    ...
    for phase = 1:apertureInfoOld.numPhases
        for i = 1:numel(apertureInfoOld.beam)
            ...
            oldLeftLeafPoss(:,i) = apertureInfoOld.beam(i).shape{phase}(1).leftLeafPos;
            oldRightLeafPoss(:,i) = apertureInfoOld.beam(i).shape{phase}(1).rightLeafPos;
        end
    end
end
    ...
    for i = 1:numel(apertureInfoOld.beam)
        ...
        for j = newInd
            ...
            if ~isfield(apertureInfoNew.beam(j),'gantryRot') || isempty(apertureInfoNew.beam(j).gantryRot)
                ...
                apertureInfoNew.beam(j).shape{phase}(1).weight = 0;
                apertureInfoNew.beam(j).shape{phase}(1).weight_I = 0;
                apertureInfoNew.beam(j).shape{phase}(1).weight_F = 0;
            end
            ...
            apertureInfoNew.beam(j).shape{phase}(1).weight = apertureInfoNew.beam(j).shape{phase}(1).weight+amountOfOldWeight*apertureInfoOld.beam(i).shape{phase}(1).weight;
            apertureInfoNew.beam(j).shape{phase}(1).weight_I = apertureInfoNew.beam(j).shape{phase}(1).weight_I+amountOfOldWeight_I*apertureInfoOld.beam(i).shape{phase}(1).weight;
            apertureInfoNew.beam(j).shape{phase}(1).weight_F = apertureInfoNew.beam(j).shape{phase}(1).weight_F+amountOfOldWeight_F*apertureInfoOld.beam(i).shape{phase}(1).weight;
            apertureInfoNew.beam(j).MU = apertureInfoNew.beam(j).shape{phase}(1).weight.*apertureInfoNew.weightToMU;
            ...
            apertureInfoNew.beam(j).shape{phase}(1).jacobiScale = 1;
            
            if recalc.interpNew
                %interpolate new apertures now so that weights are not
                %overwritten
                apertureInfoNew.beam(j).shape{phase}(1).leftLeafPos = (interp1(oldGantryAngles',oldLeftLeafPoss',apertureInfoNew.beam(j).gantryAngle))';
                apertureInfoNew.beam(j).shape{phase}(1).rightLeafPos = (interp1(oldGantryAngles',oldRightLeafPoss',apertureInfoNew.beam(j).gantryAngle))';
                
                apertureInfoNew.beam(j).shape{phase}(1).leftLeafPos_I = (interp1(oldGantryAngles',oldLeftLeafPoss',apertureInfoNew.propVMAT.beam(j).doseAngleBorders(1)))';
                apertureInfoNew.beam(j).shape{phase}(1).rightLeafPos_I = (interp1(oldGantryAngles',oldRightLeafPoss',apertureInfoNew.propVMAT.beam(j).doseAngleBorders(1)))';
                
                apertureInfoNew.beam(j).shape{phase}(1).leftLeafPos_F = (interp1(oldGantryAngles',oldLeftLeafPoss',apertureInfoNew.propVMAT.beam(j).doseAngleBorders(2)))';
                apertureInfoNew.beam(j).shape{phase}(1).rightLeafPos_F = (interp1(oldGantryAngles',oldRightLeafPoss',apertureInfoNew.propVMAT.beam(j).doseAngleBorders(2)))';
            else
                apertureInfoNew.beam(j).shape{phase}(1).leftLeafPos = apertureInfoOld.beam(i).shape{phase}(1).leftLeafPos;
                apertureInfoNew.beam(j).shape{phase}(1).rightLeafPos = apertureInfoOld.beam(i).shape{phase}(1).rightLeafPos;
            end
        ...
        if apertureInfoOld.propVMAT.beam(i).FMOBeam
            ...
            apertureInfoNew.apertureVector(shapeInd) = apertureInfoNew.beam(j).shape{phase}(1).weight;
            shapeInd = shapeInd+1;
        end
