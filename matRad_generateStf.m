...
% code
stf = struct('gantryAngle',cell(size(pln.propStf.fluGantryAngles)));
...
for i_loop = 1:numel(stf)
    
    % check if this is only a fluence calculation beam
    if ~any(abs(pln.propStf.gantryAngles - pln.propStf.fluGantryAngles(i_loop)) < 1e-6)
        % if so, there is no need to calculate dij-specific quantities
        
        % Save meta information for treatment plan
        stf(i_loop).gantryAngle   = pln.propStf.fluGantryAngles(i_loop);
        %stf(i_loop).bixelWidth    = pln.propStf.bixelWidth;
    ...
    % Save meta information for treatment plan
    stf(i_loop).gantryAngle   = pln.propStf.gantryAngles(i);
    stf(i_loop).couchAngle    = pln.propStf.couchAngles(i);
    stf(i_loop).bixelWidth    = pln.propStf.bixelWidth;
    stf(i_loop).radiationMode = pln.radiationMode;
    stf(i_loop).SAD           = SAD;
    stf(i_loop).isoCenter     = pln.propStf.isoCenter(i,:);
    ...
    if pln.propOpt.run4D
        ...
        stf(i_loop).DAD = cell(ct.numOfCtScen,1);
        offset = 0;
        normFactor = accumarray(unique2allRay,1);
        for phase = 1:ct.numOfCtScen
            
            stf(i_loop).DAD{phase} = zeros(size(rayPos_phase1));
            rayCounter = 1;
            for ray = unique2allRay'
                
                stf(i_loop).DAD{phase}(ray,1) = stf(i_loop).DAD{phase}(ray,1)+coordsAtIsoCenterPlane(offset+rayCounter,1)./normFactor(ray);
                stf(i_loop).DAD{phase}(ray,3) = stf(i_loop).DAD{phase}(ray,3)+coordsAtIsoCenterPlane(offset+rayCounter,2)./normFactor(ray);
    ...
    stf(i_loop).numOfRays = size(rayPos,1);
    
    % Save ray and target position in beam eye's view (bev)
    for j = 1:stf(i_loop).numOfRays
        stf(i_loop).ray(j).rayPos_bev = rayPos(j,:);
        stf(i_loop).ray(j).targetPoint_bev = [2*stf(i_loop).ray(j).rayPos_bev(1) ...
            SAD ...
            2*stf(i_loop).ray(j).rayPos_bev(3)];
    end
    ...
    if ~pln.propOpt.runVMAT
        ...
        stf(i_loop).sourcePoint_bev = [0 -SAD 0];
        ...
        stf(i_loop).sourcePoint = stf(i_loop).sourcePoint_bev*rotMat_vectors_T;
        
        % Save ray and target position in lps system.
        for j = 1:stf(i_loop).numOfRays
            stf(i_loop).ray(j).rayPos      = stf(i_loop).ray(j).rayPos_bev*rotMat_vectors_T;
            stf(i_loop).ray(j).targetPoint = stf(i_loop).ray(j).targetPoint_bev*rotMat_vectors_T;
            if strcmp(pln.radiationMode,'photons')
                stf(i_loop).ray(j).rayCorners_SCD = (repmat([0, machine.meta.SCD - SAD, 0],4,1)+ (machine.meta.SCD/SAD) * ...
                    [rayPos(j,:) + [+stf(i_loop).bixelWidth/2,0,+stf(i_loop).bixelWidth/2];...
                    rayPos(j,:) + [-stf(i_loop).bixelWidth/2,0,+stf(i_loop).bixelWidth/2];...
                    rayPos(j,:) + [-stf(i_loop).bixelWidth/2,0,-stf(i_loop).bixelWidth/2];...
                    rayPos(j,:) + [+stf(i_loop).bixelWidth/2,0,-stf(i_loop).bixelWidth/2]])*rotMat_vectors_T;
            end
        end
        
        % loop over all rays to determine meta information for each ray
        stf(i_loop).numOfBixelsPerRay = ones(1,stf(i_loop).numOfRays);
        
        
        for j = stf(i_loop).numOfRays:-1:1
                
                % find appropriate energies for particles
                if strcmp(stf(i_loop).radiationMode,'protons') || strcmp(stf(i_loop).radiationMode,'carbon')
                    
                    % ray tracing necessary to determine depth of the target
                    [~,l,rho,~,~] = matRad_siddonRayTracer(stf(i_loop).isoCenter, ...
                        ct.resolution, ...
                        stf(i_loop).sourcePoint, ...
                        stf(i_loop).ray(j).targetPoint, ...
                        [ct.cube {voiTarget}]);
                    ...
                    if sum(rho{2}) > 0
                        ...
                        stf(i_loop).ray(j).energy = [];
                        
                        % Save energies in stf struct
                        for k = 1:numel(targetEntry)
                            stf(i_loop).ray(j).energy = [stf(i_loop).ray(j).energy availableEnergies(availablePeakPos>=targetEntry(k)&availablePeakPos<=targetExit(k))];
                            ...
                            if strcmp(pln.machine,'HIT') && length(stf(i_loop).ray(j).energy)>2
                                Tolerance = 0.5;
                                hasNext = true;
                                CntEnergy =2;
                                while hasNext
                                    if abs(stf(i_loop).ray(j).energy(CntEnergy)-stf(i_loop).ray(j).energy(CntEnergy-1))<...
                                            DefaultLongitudialSpotSpacing-Tolerance
                                        stf(i_loop).ray(j).energy(CntEnergy)=[];
                                    else
                                        CntEnergy = CntEnergy+1;
                                    end
                                    if CntEnergy == length(stf(i_loop).ray(j).energy)
                                        hasNext = false;
                                    end
                                end
                            end
                        ...
                        stf(i_loop).numOfBixelsPerRay(j) = numel([stf(i_loop).ray(j).energy]);
                        currentMinimumFWHM = matRad_interp1(machine.meta.LUT_bxWidthminFWHM(1,:),...
                            machine.meta.LUT_bxWidthminFWHM(2,:),...
                            pln.bixelWidth);
                        focusIx  =  ones(stf(i_loop).numOfBixelsPerRay(j),1);
                        [~, vEnergyIx] = min(abs(bsxfun(@minus,[machine.data.energy]',...
                            repmat(stf(i_loop).ray(j).energy,length([machine.data]),1))));
                        
                        % get for each spot the focus index
                        for k = 1:stf(i_loop).numOfBixelsPerRay(j)
                            focusIx(k) = find(machine.data(vEnergyIx(k)).initFocus.SisFWHMAtIso > currentMinimumFWHM,1,'first');
                        end
                        
                        stf(i_loop).ray(j).focusIx = focusIx';
                        
                    else % target not hit
                        stf(i_loop).ray(j)               = [];
                        stf(i_loop).numOfBixelsPerRay(j) = [];
                    end
                    
                elseif strcmp(stf(i_loop).radiationMode,'photons')
                    
                    % book keeping for photons
                    stf(i_loop).ray(j).energy = machine.data.energy;
        ...
        stf(i_loop).numOfRays = size(stf(i_loop).ray,2);
        
        % post processing for particle remove energy slices
        if strcmp(stf(i_loop).radiationMode,'protons') || strcmp(stf(i_loop).radiationMode,'carbon')
            
            % get minimum energy per field
            minEnergy = min([stf(i_loop).ray.energy]);
            maxEnergy = max([stf(i_loop).ray.energy]);
            ...
            while ixRun <= ixEnd
                ...
                if isempty(stf(i_loop).ray(j).energy)
                    stf(i_loop).ray(j) = [];
                    stf(i_loop).numOfBixelsPerRay(j) = [];
                    stf(i_loop).numOfRays = stf(i_loop).numOfRays - 1;
                end
            end
        ...
        stf(i_loop).totalNumOfBixels = sum(stf(i_loop).numOfBixelsPerRay);
    end
    ...
    if pln.propOpt.runVMAT || (pln.propDoseCalc.vmc && strcmp(pln.propDoseCalc.vmcOptions.source,'phsp'))
        ...
        numOfRays = stf(i_loop).numOfRays;
        rayPosBEV = reshape([stf(i_loop).ray(:).rayPos_bev]',3,numOfRays)';
        ...
        if pln.propOpt.run4D
            ...
            stf(i_loop).rayPos_phase1    = rayPos_phase1;
        end
    end
    ...
    if visMode > 0
        ...
        if visMode == 2
            ...
            % voxel coordinates
            [X,Y,Z] = meshgrid((1:ct.cubeDim(2))-stf(i_loop).isoCenter(1)/ct.resolution.x, ...
                (1:ct.cubeDim(1))-stf(i_loop).isoCenter(2)/ct.resolution.y, ...
                (1:ct.cubeDim(3))-stf(i_loop).isoCenter(3)/ct.resolution.z);
        ...
        for j = 1:stf(i_loop).numOfRays
            ...
            targetPoint_vox_X_1 = stf(i_loop).ray(j).targetPoint_bev(:,1) + pln.propStf.bixelWidth;
            targetPoint_vox_Y_1 = stf(i_loop).ray(j).targetPoint_bev(:,2);
            targetPoint_vox_Z_1 = stf(i_loop).ray(j).targetPoint_bev(:,3) + pln.propStf.bixelWidth;
            
            targetPoint_vox_X_2 = stf(i_loop).ray(j).targetPoint_bev(:,1) + pln.propStf.bixelWidth;
            targetPoint_vox_Y_2 = stf(i_loop).ray(j).targetPoint_bev(:,2);
            targetPoint_vox_Z_2 = stf(i_loop).ray(j).targetPoint_bev(:,3) - pln.propStf.bixelWidth;
            
            targetPoint_vox_X_3 = stf(i_loop).ray(j).targetPoint_bev(:,1) - pln.propStf.bixelWidth;
            targetPoint_vox_Y_3 = stf(i_loop).ray(j).targetPoint_bev(:,2);
            targetPoint_vox_Z_3 = stf(i_loop).ray(j).targetPoint_bev(:,3) - pln.propStf.bixelWidth;
            
            targetPoint_vox_X_4 = stf(i_loop).ray(j).targetPoint_bev(:,1) - pln.propStf.bixelWidth;
            targetPoint_vox_Y_4 = stf(i_loop).ray(j).targetPoint_bev(:,2);
            targetPoint_vox_Z_4 = stf(i_loop).ray(j).targetPoint_bev(:,3) + pln.propStf.bixelWidth;
            
            % plot
            plot3([stf(i_loop).sourcePoint_bev(1) targetPoint_vox_X_1],[stf(i_loop).sourcePoint_bev(2) targetPoint_vox_Y_1],[stf(i_loop).sourcePoint_bev(3) targetPoint_vox_Z_1],'g')
            plot3([stf(i_loop).sourcePoint_bev(1) targetPoint_vox_X_2],[stf(i_loop).sourcePoint_bev(2) targetPoint_vox_Y_2],[stf(i_loop).sourcePoint_bev(3) targetPoint_vox_Z_2],'g')
            plot3([stf(i_loop).sourcePoint_bev(1) targetPoint_vox_X_3],[stf(i_loop).sourcePoint_bev(2) targetPoint_vox_Y_3],[stf(i_loop).sourcePoint_bev(3) targetPoint_vox_Z_3],'g')
            plot3([stf(i_loop).sourcePoint_bev(1) targetPoint_vox_X_4],[stf(i_loop).sourcePoint_bev(2) targetPoint_vox_Y_4],[stf(i_loop).sourcePoint_bev(3) targetPoint_vox_Z_4],'g')
        ...
        for j = 1:stf(i_loop).numOfRays
            ...
            targetPoint_vox_1_rotated = [stf(i_loop).ray(j).targetPoint_bev(:,1) + pln.propStf.bixelWidth,stf(i_loop).ray(j).targetPoint_bev(:,2),stf(i_loop).ray(j).targetPoint_bev(:,3) + pln.propStf.bixelWidth]*rotMat_vectors_T;
            targetPoint_vox_2_rotated = [stf(i_loop).ray(j).targetPoint_bev(:,1) + pln.propStf.bixelWidth,stf(i_loop).ray(j).targetPoint_bev(:,2),stf(i_loop).ray(j).targetPoint_bev(:,3) - pln.propStf.bixelWidth]*rotMat_vectors_T;
            targetPoint_vox_3_rotated = [stf(i_loop).ray(j).targetPoint_bev(:,1) - pln.propStf.bixelWidth,stf(i_loop).ray(j).targetPoint_bev(:,2),stf(i_loop).ray(j).targetPoint_bev(:,3) - pln.propStf.bixelWidth]*rotMat_vectors_T;
            targetPoint_vox_4_rotated = [stf(i_loop).ray(j).targetPoint_bev(:,1) - pln.propStf.bixelWidth,stf(i_loop).ray(j).targetPoint_bev(:,2),stf(i_loop).ray(j).targetPoint_bev(:,3) + pln.propStf.bixelWidth]*rotMat_vectors_T;
            
            % Plot rotated target points.
            plot3([stf(i_loop).sourcePoint(1) targetPoint_vox_1_rotated(:,1)],[stf(i_loop).sourcePoint(2) targetPoint_vox_1_rotated(:,2)],[stf(i_loop).sourcePoint(3) targetPoint_vox_1_rotated(:,3)],'g')
            plot3([stf(i_loop).sourcePoint(1) targetPoint_vox_2_rotated(:,1)],[stf(i_loop).sourcePoint(2) targetPoint_vox_2_rotated(:,2)],[stf(i_loop).sourcePoint(3) targetPoint_vox_2_rotated(:,3)],'g')
            plot3([stf(i_loop).sourcePoint(1) targetPoint_vox_3_rotated(:,1)],[stf(i_loop).sourcePoint(2) targetPoint_vox_3_rotated(:,2)],[stf(i_loop).sourcePoint(3) targetPoint_vox_3_rotated(:,3)],'g')
            plot3([stf(i_loop).sourcePoint(1) targetPoint_vox_4_rotated(:,1)],[stf(i_loop).sourcePoint(2) targetPoint_vox_4_rotated(:,2)],[stf(i_loop).sourcePoint(3) targetPoint_vox_4_rotated(:,3)],'g')
        end
    ...
    if strcmp(pln.radiationMode, 'protons') || strcmp(pln.radiationMode, 'carbon')
        for j = 1:stf(i_loop).numOfRays
            for k = 1:numel(stf(i_loop).ray(j).energy)
                stf(i_loop).ray(j).rangeShifter(k).ID = 0;
                stf(i_loop).ray(j).rangeShifter(k).eqThickness = 0;
                stf(i_loop).ray(j).rangeShifter(k).sourceRashiDistance = 0;
            end
        end
    end
...
if pln.propDoseCalc.vmc
    if strcmp(pln.propDoseCalc.vmcOptions.source,'phsp')
        stf = matRad_bixelPhspVmc(stf,masterRayPosBEV,pln.propDoseCalc.vmcOptions,SAD,pln.propStf.bixelWidth);
    end
else
